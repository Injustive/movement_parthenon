...
import curl_cffi
from .utils import get_random_avatar, generate_additional_wallet, NoTokenException
...
class Task(Logger, ModernTask):
    def __init__(self,
        ...
        self.aptos_client = AptosClient(rest_api_url=RpcProviders.MOVEMENT_MAINNET.value,
                                        proxy=self.session.proxies.get('http')).client
        self.jwt = jwt_token
        self.pair = pair
    ...
    async def connect_discord(self):
        ...
        raise NoTokenException("Discord token not found!")
    ...
    async def connect_twitter(self):
        ...
        raise NoTokenException("Twitter token not found")
